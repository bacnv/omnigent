...
describe("NewChatLandingScreen", () => {
  ...
  it("sends the selected Codex launch model without changing Claude's remembered model", async () => {
    ...
    // picker starts from its own Sonnet default instead of inheriting the GPT id.
    openAgentConfig("a1");
    expect(screen.getByTestId("new-chat-landing-config-model").textContent).toContain("Sonnet 4.6");
...
describe("NewChatLandingScreen smart routing", () => {
  ...
  it("remembers Codex's routing pick without touching Claude Code's", () => {
    ...
    closeMenu();
    // Claude Code never had anything stored, so it starts on its own Sonnet default.
    openAgentConfig("a1");
