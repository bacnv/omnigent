...
async def test_refresh_loop_mints_only_after_sleep_then_restamps(
    ...
    rewritten: list[tuple[Path, dict[str, str]]] = []

    def _refresh(bridge_dir: Path, headers: dict[str, str]) -> bool:
        rewritten.append((bridge_dir, headers))
        return True
    ...
    task = asyncio.create_task(
        _refresh_claude_permission_hook_auth(_BRIDGE_DIR, _SERVER_URL, _factory, sleep=gate)
    )
    ...
    try:
        ...
        assert rewritten == [
            (
                _BRIDGE_DIR,
                {"Authorization": "Bearer tok0", "X-Server": _SERVER_URL},
            )
        ], rewritten
